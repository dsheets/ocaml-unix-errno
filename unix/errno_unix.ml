...
let optional_unknown ~host errno =
  match Errno.to_code ~host errno with
    | Some i -> Some (Unix.EUNKNOWNERR (Signed.SInt.to_int i))
    | None -> None

let to_unix ?(host = host) =
  Errno.(
    function
...
let rec unix_error_of_errno = function
  | [] -> None
  | err :: rest -> (
      match to_unix err with
        | Some err -> Some err
        | None -> unix_error_of_errno rest)

let to_unix_exn = function
  | Errno.Error { Errno.errno; call; label } as e -> begin
      match unix_error_of_errno errno with
