...
let field_of_errno errno = String.lowercase_ascii (Errno.to_string errno)

let () =
  Errno.iter_defns defns
    (fun i errno ->
      printf "  %s = Some (Signed.SInt.of_int64 %LdL);\n" (field_of_errno errno)
        (Signed.SInt.to_int64 i))
