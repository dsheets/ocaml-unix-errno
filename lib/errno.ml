...
let with_code defns symbol code =
  match symbol with
...
let iter_defns defns f_exist f_missing =
  (match defns.e2big with Some x -> f_exist x E2BIG | None -> f_missing E2BIG);
  (match defns.eacces with
    | Some x -> f_exist x EACCES
    | None -> f_missing EACCES);
  (match defns.eaddrinuse with
    | Some x -> f_exist x EADDRINUSE
    | None -> f_missing EADDRINUSE);
  (match defns.eaddrnotavail with
    | Some x -> f_exist x EADDRNOTAVAIL
    | None -> f_missing EADDRNOTAVAIL);
  (match defns.eafnosupport with
    | Some x -> f_exist x EAFNOSUPPORT
    | None -> f_missing EAFNOSUPPORT);
  (match defns.eagain with
    | Some x -> f_exist x EAGAIN
    | None -> f_missing EAGAIN);
  (match defns.ealready with
    | Some x -> f_exist x EALREADY
    | None -> f_missing EALREADY);
  (match defns.ebadf with Some x -> f_exist x EBADF | None -> f_missing EBADF);
  (match defns.ebadmsg with
    | Some x -> f_exist x EBADMSG
    | None -> f_missing EBADMSG);
  (match defns.ebusy with Some x -> f_exist x EBUSY | None -> f_missing EBUSY);
  (match defns.ecanceled with
    | Some x -> f_exist x ECANCELED
    | None -> f_missing ECANCELED);
  (match defns.echild with
    | Some x -> f_exist x ECHILD
    | None -> f_missing ECHILD);
  (match defns.econnaborted with
    | Some x -> f_exist x ECONNABORTED
    | None -> f_missing ECONNABORTED);
  (match defns.econnrefused with
    | Some x -> f_exist x ECONNREFUSED
    | None -> f_missing ECONNREFUSED);
  (match defns.econnreset with
    | Some x -> f_exist x ECONNRESET
    | None -> f_missing ECONNRESET);
  (match defns.edeadlk with
    | Some x -> f_exist x EDEADLK
    | None -> f_missing EDEADLK);
  (match defns.edestaddrreq with
    | Some x -> f_exist x EDESTADDRREQ
    | None -> f_missing EDESTADDRREQ);
  (match defns.edom with Some x -> f_exist x EDOM | None -> f_missing EDOM);
  (match defns.edquot with
    | Some x -> f_exist x EDQUOT
    | None -> f_missing EDQUOT);
  (match defns.eexist with
    | Some x -> f_exist x EEXIST
    | None -> f_missing EEXIST);
  (match defns.efault with
    | Some x -> f_exist x EFAULT
    | None -> f_missing EFAULT);
  (match defns.efbig with Some x -> f_exist x EFBIG | None -> f_missing EFBIG);
  (match defns.ehostdown with
    | Some x -> f_exist x EHOSTDOWN
    | None -> f_missing EHOSTDOWN);
  (match defns.ehostunreach with
    | Some x -> f_exist x EHOSTUNREACH
    | None -> f_missing EHOSTUNREACH);
  (match defns.eidrm with Some x -> f_exist x EIDRM | None -> f_missing EIDRM);
  (match defns.eilseq with
    | Some x -> f_exist x EILSEQ
    | None -> f_missing EILSEQ);
  (match defns.einprogress with
    | Some x -> f_exist x EINPROGRESS
    | None -> f_missing EINPROGRESS);
  (match defns.eintr with Some x -> f_exist x EINTR | None -> f_missing EINTR);
  (match defns.einval with
    | Some x -> f_exist x EINVAL
    | None -> f_missing EINVAL);
  (match defns.eio with Some x -> f_exist x EIO | None -> f_missing EIO);
  (match defns.eisconn with
    | Some x -> f_exist x EISCONN
    | None -> f_missing EISCONN);
  (match defns.eisdir with
    | Some x -> f_exist x EISDIR
    | None -> f_missing EISDIR);
  (match defns.eloop with Some x -> f_exist x ELOOP | None -> f_missing ELOOP);
  (match defns.emfile with
    | Some x -> f_exist x EMFILE
    | None -> f_missing EMFILE);
  (match defns.emlink with
    | Some x -> f_exist x EMLINK
    | None -> f_missing EMLINK);
  (match defns.emsgsize with
    | Some x -> f_exist x EMSGSIZE
    | None -> f_missing EMSGSIZE);
  (match defns.emultihop with
    | Some x -> f_exist x EMULTIHOP
    | None -> f_missing EMULTIHOP);
  (match defns.enametoolong with
    | Some x -> f_exist x ENAMETOOLONG
    | None -> f_missing ENAMETOOLONG);
  (match defns.enetdown with
    | Some x -> f_exist x ENETDOWN
    | None -> f_missing ENETDOWN);
  (match defns.enetreset with
    | Some x -> f_exist x ENETRESET
    | None -> f_missing ENETRESET);
  (match defns.enetunreach with
    | Some x -> f_exist x ENETUNREACH
    | None -> f_missing ENETUNREACH);
  (match defns.enfile with
    | Some x -> f_exist x ENFILE
    | None -> f_missing ENFILE);
  (match defns.enobufs with
    | Some x -> f_exist x ENOBUFS
    | None -> f_missing ENOBUFS);
  (match defns.enodev with
    | Some x -> f_exist x ENODEV
    | None -> f_missing ENODEV);
  (match defns.enoent with
    | Some x -> f_exist x ENOENT
    | None -> f_missing ENOENT);
  (match defns.enoexec with
    | Some x -> f_exist x ENOEXEC
    | None -> f_missing ENOEXEC);
  (match defns.enolck with
    | Some x -> f_exist x ENOLCK
    | None -> f_missing ENOLCK);
  (match defns.enolink with
    | Some x -> f_exist x ENOLINK
    | None -> f_missing ENOLINK);
  (match defns.enomem with
    | Some x -> f_exist x ENOMEM
    | None -> f_missing ENOMEM);
  (match defns.enomsg with
    | Some x -> f_exist x ENOMSG
    | None -> f_missing ENOMSG);
  (match defns.enoprotoopt with
    | Some x -> f_exist x ENOPROTOOPT
    | None -> f_missing ENOPROTOOPT);
  (match defns.enospc with
    | Some x -> f_exist x ENOSPC
    | None -> f_missing ENOSPC);
  (match defns.enosys with
    | Some x -> f_exist x ENOSYS
    | None -> f_missing ENOSYS);
  (match defns.enotblk with
    | Some x -> f_exist x ENOTBLK
    | None -> f_missing ENOTBLK);
  (match defns.enotconn with
    | Some x -> f_exist x ENOTCONN
    | None -> f_missing ENOTCONN);
  (match defns.enotdir with
    | Some x -> f_exist x ENOTDIR
    | None -> f_missing ENOTDIR);
  (match defns.enotempty with
    | Some x -> f_exist x ENOTEMPTY
    | None -> f_missing ENOTEMPTY);
  (match defns.enotrecoverable with
    | Some x -> f_exist x ENOTRECOVERABLE
    | None -> f_missing ENOTRECOVERABLE);
  (match defns.enotsock with
    | Some x -> f_exist x ENOTSOCK
    | None -> f_missing ENOTSOCK);
  (match defns.enotsup with
    | Some x -> f_exist x ENOTSUP
    | None -> f_missing ENOTSUP);
  (match defns.enotty with
    | Some x -> f_exist x ENOTTY
    | None -> f_missing ENOTTY);
  (match defns.enxio with Some x -> f_exist x ENXIO | None -> f_missing ENXIO);
  (match defns.eopnotsupp with
    | Some x -> f_exist x EOPNOTSUPP
    | None -> f_missing EOPNOTSUPP);
  (match defns.eoverflow with
    | Some x -> f_exist x EOVERFLOW
    | None -> f_missing EOVERFLOW);
  (match defns.eownerdead with
    | Some x -> f_exist x EOWNERDEAD
    | None -> f_missing EOWNERDEAD);
  (match defns.eperm with Some x -> f_exist x EPERM | None -> f_missing EPERM);
  (match defns.epfnosupport with
    | Some x -> f_exist x EPFNOSUPPORT
    | None -> f_missing EPFNOSUPPORT);
  (match defns.epipe with Some x -> f_exist x EPIPE | None -> f_missing EPIPE);
  (match defns.eproto with
    | Some x -> f_exist x EPROTO
    | None -> f_missing EPROTO);
  (match defns.eprotonosupport with
    | Some x -> f_exist x EPROTONOSUPPORT
    | None -> f_missing EPROTONOSUPPORT);
  (match defns.eprototype with
    | Some x -> f_exist x EPROTOTYPE
    | None -> f_missing EPROTOTYPE);
  (match defns.erange with
    | Some x -> f_exist x ERANGE
    | None -> f_missing ERANGE);
  (match defns.eremote with
    | Some x -> f_exist x EREMOTE
    | None -> f_missing EREMOTE);
  (match defns.erofs with Some x -> f_exist x EROFS | None -> f_missing EROFS);
  (match defns.eshutdown with
    | Some x -> f_exist x ESHUTDOWN
    | None -> f_missing ESHUTDOWN);
  (match defns.esocktnosupport with
    | Some x -> f_exist x ESOCKTNOSUPPORT
    | None -> f_missing ESOCKTNOSUPPORT);
  (match defns.espipe with
    | Some x -> f_exist x ESPIPE
    | None -> f_missing ESPIPE);
  (match defns.esrch with Some x -> f_exist x ESRCH | None -> f_missing ESRCH);
  (match defns.estale with
    | Some x -> f_exist x ESTALE
    | None -> f_missing ESTALE);
  (match defns.etimedout with
    | Some x -> f_exist x ETIMEDOUT
    | None -> f_missing ETIMEDOUT);
  (match defns.etoomanyrefs with
    | Some x -> f_exist x ETOOMANYREFS
    | None -> f_missing ETOOMANYREFS);
  (match defns.etxtbsy with
    | Some x -> f_exist x ETXTBSY
    | None -> f_missing ETXTBSY);
  (match defns.eusers with
    | Some x -> f_exist x EUSERS
    | None -> f_missing EUSERS);
  (match defns.ewouldblock with
    | Some x -> f_exist x EWOULDBLOCK
    | None -> f_missing EWOULDBLOCK);
  (match defns.exdev with Some x -> f_exist x EXDEV | None -> f_missing EXDEV);
  (match defns.echrng with
    | Some x -> f_exist x ECHRNG
    | None -> f_missing ECHRNG);
  (match defns.el2nsync with
    | Some x -> f_exist x EL2NSYNC
    | None -> f_missing EL2NSYNC);
  (match defns.el3hlt with
    | Some x -> f_exist x EL3HLT
    | None -> f_missing EL3HLT);
  (match defns.el3rst with
    | Some x -> f_exist x EL3RST
    | None -> f_missing EL3RST);
  (match defns.elnrng with
    | Some x -> f_exist x ELNRNG
    | None -> f_missing ELNRNG);
  (match defns.eunatch with
    | Some x -> f_exist x EUNATCH
    | None -> f_missing EUNATCH);
  (match defns.enocsi with
    | Some x -> f_exist x ENOCSI
    | None -> f_missing ENOCSI);
  (match defns.el2hlt with
    | Some x -> f_exist x EL2HLT
    | None -> f_missing EL2HLT);
  (match defns.ebade with Some x -> f_exist x EBADE | None -> f_missing EBADE);
  (match defns.ebadr with Some x -> f_exist x EBADR | None -> f_missing EBADR);
  (match defns.exfull with
    | Some x -> f_exist x EXFULL
    | None -> f_missing EXFULL);
  (match defns.enoano with
    | Some x -> f_exist x ENOANO
    | None -> f_missing ENOANO);
  (match defns.ebadrqc with
    | Some x -> f_exist x EBADRQC
    | None -> f_missing EBADRQC);
  (match defns.ebadslt with
    | Some x -> f_exist x EBADSLT
    | None -> f_missing EBADSLT);
  (match defns.ebfont with
    | Some x -> f_exist x EBFONT
    | None -> f_missing EBFONT);
  (match defns.enonet with
    | Some x -> f_exist x ENONET
    | None -> f_missing ENONET);
  (match defns.enopkg with
    | Some x -> f_exist x ENOPKG
    | None -> f_missing ENOPKG);
  (match defns.eadv with Some x -> f_exist x EADV | None -> f_missing EADV);
  (match defns.esrmnt with
    | Some x -> f_exist x ESRMNT
    | None -> f_missing ESRMNT);
  (match defns.ecomm with Some x -> f_exist x ECOMM | None -> f_missing ECOMM);
  (match defns.edotdot with
    | Some x -> f_exist x EDOTDOT
    | None -> f_missing EDOTDOT);
  (match defns.enotuniq with
    | Some x -> f_exist x ENOTUNIQ
    | None -> f_missing ENOTUNIQ);
  (match defns.ebadfd with
    | Some x -> f_exist x EBADFD
    | None -> f_missing EBADFD);
  (match defns.eremchg with
    | Some x -> f_exist x EREMCHG
    | None -> f_missing EREMCHG);
  (match defns.elibacc with
    | Some x -> f_exist x ELIBACC
    | None -> f_missing ELIBACC);
  (match defns.elibbad with
    | Some x -> f_exist x ELIBBAD
    | None -> f_missing ELIBBAD);
  (match defns.elibscn with
    | Some x -> f_exist x ELIBSCN
    | None -> f_missing ELIBSCN);
  (match defns.elibmax with
    | Some x -> f_exist x ELIBMAX
    | None -> f_missing ELIBMAX);
  (match defns.elibexec with
    | Some x -> f_exist x ELIBEXEC
    | None -> f_missing ELIBEXEC);
  (match defns.erestart with
    | Some x -> f_exist x ERESTART
    | None -> f_missing ERESTART);
  (match defns.estrpipe with
    | Some x -> f_exist x ESTRPIPE
    | None -> f_missing ESTRPIPE);
  (match defns.euclean with
    | Some x -> f_exist x EUCLEAN
    | None -> f_missing EUCLEAN);
  (match defns.enotnam with
    | Some x -> f_exist x ENOTNAM
    | None -> f_missing ENOTNAM);
  (match defns.enavail with
    | Some x -> f_exist x ENAVAIL
    | None -> f_missing ENAVAIL);
  (match defns.eisnam with
    | Some x -> f_exist x EISNAM
    | None -> f_missing EISNAM);
  (match defns.eremoteio with
    | Some x -> f_exist x EREMOTEIO
    | None -> f_missing EREMOTEIO);
  (match defns.enomedium with
    | Some x -> f_exist x ENOMEDIUM
    | None -> f_missing ENOMEDIUM);
  (match defns.emediumtype with
    | Some x -> f_exist x EMEDIUMTYPE
    | None -> f_missing EMEDIUMTYPE);
  (match defns.enokey with
    | Some x -> f_exist x ENOKEY
    | None -> f_missing ENOKEY);
  (match defns.ekeyexpired with
    | Some x -> f_exist x EKEYEXPIRED
    | None -> f_missing EKEYEXPIRED);
  (match defns.ekeyrevoked with
    | Some x -> f_exist x EKEYREVOKED
    | None -> f_missing EKEYREVOKED);
  (match defns.ekeyrejected with
    | Some x -> f_exist x EKEYREJECTED
    | None -> f_missing EKEYREJECTED);
  (match defns.erfkill with
    | Some x -> f_exist x ERFKILL
    | None -> f_missing ERFKILL);
  (match defns.ehwpoison with
    | Some x -> f_exist x EHWPOISON
    | None -> f_missing EHWPOISON);
  (match defns.epwroff with
    | Some x -> f_exist x EPWROFF
    | None -> f_missing EPWROFF);
  (match defns.edeverr with
    | Some x -> f_exist x EDEVERR
    | None -> f_missing EDEVERR);
  (match defns.ebadexec with
    | Some x -> f_exist x EBADEXEC
    | None -> f_missing EBADEXEC);
  (match defns.ebadarch with
    | Some x -> f_exist x EBADARCH
    | None -> f_missing EBADARCH);
  (match defns.eshlibvers with
    | Some x -> f_exist x ESHLIBVERS
    | None -> f_missing ESHLIBVERS);
  (match defns.ebadmacho with
    | Some x -> f_exist x EBADMACHO
    | None -> f_missing EBADMACHO);
  (match defns.enopolicy with
    | Some x -> f_exist x ENOPOLICY
    | None -> f_missing ENOPOLICY);
  (match defns.eqfull with
    | Some x -> f_exist x EQFULL
    | None -> f_missing EQFULL);
  (match defns.edoofus with
    | Some x -> f_exist x EDOOFUS
    | None -> f_missing EDOOFUS);
  (match defns.enotcapable with
    | Some x -> f_exist x ENOTCAPABLE
    | None -> f_missing ENOTCAPABLE);
  (match defns.ecapmode with
    | Some x -> f_exist x ECAPMODE
    | None -> f_missing ECAPMODE);
  (match defns.eproclim with
    | Some x -> f_exist x EPROCLIM
    | None -> f_missing EPROCLIM);
  (match defns.ebadrpc with
    | Some x -> f_exist x EBADRPC
    | None -> f_missing EBADRPC);
  (match defns.erpcmismatch with
    | Some x -> f_exist x ERPCMISMATCH
    | None -> f_missing ERPCMISMATCH);
  (match defns.eprogunavail with
    | Some x -> f_exist x EPROGUNAVAIL
    | None -> f_missing EPROGUNAVAIL);
  (match defns.eprogmismatch with
    | Some x -> f_exist x EPROGMISMATCH
    | None -> f_missing EPROGMISMATCH);
  (match defns.eprocunavail with
    | Some x -> f_exist x EPROCUNAVAIL
    | None -> f_missing EPROCUNAVAIL);
  (match defns.eftype with
    | Some x -> f_exist x EFTYPE
    | None -> f_missing EFTYPE);
  (match defns.eauth with Some x -> f_exist x EAUTH | None -> f_missing EAUTH);
  (match defns.eneedauth with
    | Some x -> f_exist x ENEEDAUTH
    | None -> f_missing ENEEDAUTH);
  (match defns.enoattr with
    | Some x -> f_exist x ENOATTR
    | None -> f_missing ENOATTR);
  (match defns.enostr with
    | Some x -> f_exist x ENOSTR
    | None -> f_missing ENOSTR);
  (match defns.enodata with
    | Some x -> f_exist x ENODATA
    | None -> f_missing ENODATA);
  (match defns.etime with Some x -> f_exist x ETIME | None -> f_missing ETIME);
  (match defns.enosr with Some x -> f_exist x ENOSR | None -> f_missing ENOSR);
  ()
    ...
    try
      let symbol, code, off =
        Scanf.sscanf s "%s\t%s\n" (fun symbol_s code_s ->
            ( of_string symbol_s,
              (if code_s = "" then None
              else Some (Signed.SInt.of_string code_s)),
...
let check_errno fn = try Result.Ok (fn ()) with Error e -> Result.Error e
